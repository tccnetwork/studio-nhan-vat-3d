...
def build_silky_soft_hair_5_states():
    ...
    if orig_hair:
        ...
        def create_tapered_hairstyle(z_back, z_side, name, u_curve_depth=0.038,
            ...
            tuck            : mức thu vào của đuôi tóc, 0 = giữ nguyên độ xoè
            """
            ...
            for v in me.vertices:
                ...
                # Đuôi tóc thu vào một chút cho khỏi xoè, nhưng chặn ở vỏ sọ:
                # tóc không bao giờ được phép mảnh hơn chính cái đầu nó bọc.
                if tuck > 0.0:
                    dx, dy = v.co.x, v.co.y - AXIS_Y
                    r = (dx * dx + dy * dy) ** 0.5
                    if r > 1e-5:
                        k = new_span / orig_span
                        shrink = 1.0 - (1.0 - k) * t * tuck
                        r_floor = min(r, head_radius(v.co.z) + 0.014)
                        r_new = max(r * shrink, r_floor)
                        scale = r_new / r
                        v.co.x = dx * scale
