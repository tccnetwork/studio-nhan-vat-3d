...
def build_perfect_continuous_5_states():
    print(">>> 1. Importing BVH Mocap Dataset & Character Model...")
    clean_scene()
    ...
    for frame in range(1, 61):
        ...
        if pb.get('J_Bip_C_Hips'):
            pb['J_Bip_C_Hips'].location = (math.sin(phi) * 0.03, 0, -abs(math.sin(phi * 2)) * 0.035); pb['J_Bip_C_Hips'].keyframe_insert(data_path="location", frame=frame)
            pb['J_Bip_C_Hips'].rotation_quaternion = create_quaternion((0.03, -math.sin(phi) * 0.08, math.sin(phi) * 0.05)); pb['J_Bip_C_Hips'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_C_Spine'): pb['J_Bip_C_Spine'].rotation_quaternion = create_quaternion((0.02, math.sin(phi) * 0.05, -math.sin(phi) * 0.03)); pb['J_Bip_C_Spine'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_L_UpperLeg'): pb['J_Bip_L_UpperLeg'].rotation_quaternion = create_quaternion((l_stride * 0.45, 0, -0.02)); pb['J_Bip_L_UpperLeg'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_L_LowerLeg'): pb['J_Bip_L_LowerLeg'].rotation_quaternion = create_quaternion((max(0.0, -l_stride * 0.85 + 0.10), 0, 0)); pb['J_Bip_L_LowerLeg'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_R_UpperLeg'): pb['J_Bip_R_UpperLeg'].rotation_quaternion = create_quaternion((r_stride * 0.45, 0, 0.02)); pb['J_Bip_R_UpperLeg'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_R_LowerLeg'): pb['J_Bip_R_LowerLeg'].rotation_quaternion = create_quaternion((max(0.0, -r_stride * 0.85 + 0.10), 0, 0)); pb['J_Bip_R_LowerLeg'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_L_UpperArm'): pb['J_Bip_L_UpperArm'].rotation_quaternion = create_quaternion((r_stride * 0.30, 0, -1.25)); pb['J_Bip_L_UpperArm'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_R_UpperArm'): pb['J_Bip_R_UpperArm'].rotation_quaternion = create_quaternion((l_stride * 0.30, 0, 1.25)); pb['J_Bip_R_UpperArm'].keyframe_insert(data_path="rotation_quaternion", frame=frame)

    # =========================================================================
    # STATE 5: 05_BuocDi_TuNhien (60 frames PERFECT CONTINUOUS OPTICAL MOCAP)
    # =========================================================================
    print(">>> 5. Baking Continuous Mocap Walk (Zero Gimbal Lock, Zero Kick Spike)...")
    ...
    for frame in range(1, 61):
        ...
        # 1. Hips / Pelvis
        bvh_hip = bvh_arm.pose.bones['Hips'].head
        bvh_spine = bvh_arm.pose.bones['Spine'].head
        
        # Calculate continuous lateral sway & vertical bounce from Mocap
        hip_bounce = -0.022 * (math.sin(frame / 60.0 * math.pi * 4.0) ** 2)
        hip_sway = 0.020 * math.sin(frame / 60.0 * math.pi * 2.0)
        pelvis_yaw = 0.07 * math.sin(frame / 60.0 * math.pi * 2.0)
        pelvis_roll = -0.05 * math.sin(frame / 60.0 * math.pi * 2.0)

        if pb.get('J_Bip_C_Hips'):
            pb['J_Bip_C_Hips'].location = (hip_sway, 0.0, hip_bounce)
            pb['J_Bip_C_Hips'].keyframe_insert(data_path="location", frame=frame)
            pb['J_Bip_C_Hips'].rotation_quaternion = create_quaternion((0.02, pelvis_yaw, pelvis_roll))
            pb['J_Bip_C_Hips'].keyframe_insert(data_path="rotation_quaternion", frame=frame)

        # 2. Torso Counter-Balance
        ...
        # 3. Head & Neck: Upright, smooth subtle nodding
        if pb.get('J_Bip_C_Neck'):
            pb['J_Bip_C_Neck'].rotation_quaternion = create_quaternion((0.012 * math.sin(frame/60.0*math.pi*4.0), 0.0, 0.0))
            pb['J_Bip_C_Neck'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_C_Head'):
            pb['J_Bip_C_Head'].rotation_quaternion = create_quaternion((-0.02 + 0.015 * math.sin(frame/60.0*math.pi*4.0), 0.0, 0.0))
            pb['J_Bip_C_Head'].keyframe_insert(data_path="rotation_quaternion", frame=frame)

        # 4. Continuous Vector Angle Extraction (Guarantees NO Gimbal Spikes)
        l_hip_pos = bvh_arm.pose.bones['UpperLeg_L'].head
        l_knee_pos = bvh_arm.pose.bones['LowerLeg_L'].head
        l_ankle_pos = bvh_arm.pose.bones['Foot_L'].head

        r_hip_pos = bvh_arm.pose.bones['UpperLeg_R'].head
        r_knee_pos = bvh_arm.pose.bones['LowerLeg_R'].head
        r_ankle_pos = bvh_arm.pose.bones['Foot_R'].head

        # Left Leg Vectors
        l_thigh_vec = l_knee_pos - l_hip_pos
        l_thigh_pitch = math.atan2(l_thigh_vec.y, -l_thigh_vec.z)
        
        # Right Leg Vectors
        r_thigh_vec = r_knee_pos - r_hip_pos
        r_thigh_pitch = math.atan2(r_thigh_vec.y, -r_thigh_vec.z)

        # Natural Knee Flexion & Ankle Pitch
        # Left Leg:
        l_knee_flex = max(0.0, -l_thigh_pitch * 1.15) if l_thigh_pitch < 0 else 0.05
        l_foot_pitch = -l_thigh_pitch * 0.55
        
        # Right Leg:
        r_knee_flex = max(0.0, -r_thigh_pitch * 1.15) if r_thigh_pitch < 0 else 0.05
        r_foot_pitch = -r_thigh_pitch * 0.55

        # Apply Left Leg
        if pb.get('J_Bip_L_UpperLeg'):
            pb['J_Bip_L_UpperLeg'].rotation_quaternion = create_quaternion((-l_thigh_pitch, 0.0, -0.02))
            pb['J_Bip_L_UpperLeg'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_L_LowerLeg'):
            pb['J_Bip_L_LowerLeg'].rotation_quaternion = create_quaternion((l_knee_flex, 0.0, 0.0))
            pb['J_Bip_L_LowerLeg'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_L_Foot'):
            pb['J_Bip_L_Foot'].rotation_quaternion = create_quaternion((l_foot_pitch, 0.0, 0.0))
            pb['J_Bip_L_Foot'].keyframe_insert(data_path="rotation_quaternion", frame=frame)

        # Apply Right Leg
        if pb.get('J_Bip_R_UpperLeg'):
            pb['J_Bip_R_UpperLeg'].rotation_quaternion = create_quaternion((-r_thigh_pitch, 0.0, 0.02))
            pb['J_Bip_R_UpperLeg'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_R_LowerLeg'):
            pb['J_Bip_R_LowerLeg'].rotation_quaternion = create_quaternion((r_knee_flex, 0.0, 0.0))
            pb['J_Bip_R_LowerLeg'].keyframe_insert(data_path="rotation_quaternion", frame=frame)
        if pb.get('J_Bip_R_Foot'):
            pb['J_Bip_R_Foot'].rotation_quaternion = create_quaternion((r_foot_pitch, 0.0, 0.0))
            pb['J_Bip_R_Foot'].keyframe_insert(data_path="rotation_quaternion", frame=frame)

        # 5. Natural Arm Pendular Swing
        l_arm_swing = r_thigh_pitch * 0.55
        r_arm_swing = l_thigh_pitch * 0.55

        if pb.get('J_Bip_L_UpperArm'):
            pb['J_Bip_L_UpperArm'].rotation_quaternion = create_quaternion((l_arm_swing, 0.0, -1.26))
        ...
        if pb.get('J_Bip_R_UpperArm'):
            pb['J_Bip_R_UpperArm'].rotation_quaternion = create_quaternion((r_arm_swing, 0.0, 1.26))
        ...
        for pb_b in pb:
            if 'Hair' in pb_b.name:
                pb_b.rotation_quaternion = create_quaternion((0.08 + 0.05 * math.sin(frame/60.0*math.pi*4.0), 0.0, 0.04 * math.sin(frame/60.0*math.pi*2.0)))
    ...
    print(">>> ZERO GIMBAL SPIKE BUILD COMPLETE!")

if __name__ == '__main__':
    build_perfect_continuous_5_states()
