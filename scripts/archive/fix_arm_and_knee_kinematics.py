...
# -------------------------------------------------------------------------
# MODEL B (ANIME IDOL): FIX ARMS DOWN FROM T-POSE + DEEP KNEE BENDS
# -------------------------------------------------------------------------
def fix_model_B():
    print(">>> Baking Realistic Arm Gestures & Deep Knee Bends for Model B (Anime)...")
    clean_scene()
    ...
    action = bpy.data.actions.new(name="01_Idol_ActiveArms_DeepKneeBends")
    ...
    for frame in range(1, 241):
        ...
        beat_t = (frame / 30.0) * math.pi * 4.33 # J-Pop 130 BPM
        
        # 1. HIPS: Dynamic bounce & sway
        hips = armature.pose.bones.get('J_Bip_C_Hips')
        if hips:
            hips.location.x = math.sin(beat_t * 0.5) * 0.05
            hips.location.z = -abs(math.sin(beat_t)) * 0.08 # Deep bounce!
        ...
        if l_up and l_low:
            ...
            l_low_euler = mathutils.Euler((knee_bend_L * 1.40, 0.0, 0.0), 'XYZ')
            ...
            if l_ft:
                l_ft_euler = mathutils.Euler((-knee_bend_L * 0.75, 0.0, 0.0), 'XYZ')
        ...
        if r_up and r_low:
            ...
            r_low_euler = mathutils.Euler((knee_bend_R * 1.40, 0.0, 0.0), 'XYZ')
            ...
            if r_ft:
                r_ft_euler = mathutils.Euler((-knee_bend_R * 0.75, 0.0, 0.0), 'XYZ')
        ...
        # 4. ARMS & FOREARMS (KHẮC PHỤC DANG TAY T-POSE - CỬ ĐỘNG TAY CA SĨ SỐNG ĐỘNG)
        ...
        # RIGHT ARM (Holding Stage Mic at mouth):
        if r_arm and r_fore:
            # Drop from T-pose (Z: +1.15 rad) + forward (Y: +0.65 rad)
            r_arm_euler = mathutils.Euler((
                0.0,
            ...
            # Right Elbow BENT UP holding mic to mouth (Z: +1.25 rad, Y: +0.25 rad)
            r_fore_euler = mathutils.Euler((
                0.0,
                0.25,
                1.25 + math.sin(beat_t) * 0.12
            ), 'XYZ')
            ...
            if r_hand:
                r_hand_euler = mathutils.Euler((0.0, 0.3, 0.2), 'XYZ')
                r_hand.rotation_mode = 'QUATERNION'
                r_hand.rotation_quaternion = r_hand_euler.to_quaternion()
                r_hand.keyframe_insert(data_path="rotation_quaternion", frame=frame)

        # LEFT ARM (Expressive Idol Gestures & Stage Wave):
        if l_arm and l_fore:
            idol_cycle = (frame % 90) / 90.0 * math.pi * 2
            
            # Drop from T-pose (Z: -1.15 rad) + smooth sweeping gestures
            l_arm_euler = mathutils.Euler((
                0.0,
            ...
            # Left Elbow actively flexing and extending!
            l_fore_euler = mathutils.Euler((
                0.0,
                -0.20,
                -0.85 - abs(math.sin(idol_cycle)) * 0.50
            ), 'XYZ')
            ...
            if l_hand:
                l_hand_euler = mathutils.Euler((0.0, -0.2, -0.3 + math.sin(idol_cycle * 2) * 0.2), 'XYZ')
                l_hand.rotation_mode = 'QUATERNION'
                l_hand.rotation_quaternion = l_hand_euler.to_quaternion()
    ...
    print(">>> Model B Exported with Fixed Arm Down Gestures & Knee Bends!")

if __name__ == '__main__':
    fix_model_B()
    print("=== MODEL B CHOREOGRAPHY UPDATED! ===")
