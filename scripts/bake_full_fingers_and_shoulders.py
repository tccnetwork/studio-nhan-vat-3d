...
def bake_shoulders_fingers_and_full_performance():
    print(">>> Baking Advanced Shoulder & Finger Articulations...")
    clean_scene()
    ...
    # 1. BIND CLOTHING VERTEX GROUPS (ÁO & QUẦN BÁM CHẶT KHUNG XƯƠNG)
    body_obj = None
    ...
    action = bpy.data.actions.new(name="01_Idol_FullArticulated_Choreography")
    ...
    for frame in range(1, 241):
        ...
        # 4. SHOULDERS (KHỚP VAI BIỂU DIỄN & NHẤP NHÔ THEO NHỊP HÁT)
        l_sh = armature.pose.bones.get('J_Bip_L_Shoulder')
        r_sh = armature.pose.bones.get('J_Bip_R_Shoulder')
        
        if r_sh:
            # Right shoulder lifts slightly to support mic arm + breathes with vocals
        ...
        if l_sh:
            # Left shoulder rhythmically rolls with idol gestures
            l_sh.rotation_mode = 'QUATERNION'
            l_sh.rotation_quaternion = mathutils.Euler((
                0.0,
                -0.03 + math.sin(idol_cycle) * 0.04,
        ...
        # -------------------------------------------------------------
        # 6. FINGERS ARTICULATION (CÁC KHỚP NGÓN TAY CẦM MICRO & DIỄN CẢM)
        # -------------------------------------------------------------
        # A. RIGHT HAND FINGERS: Firm natural grip holding the microphone cylinder
        grip_pulse = 0.05 * math.sin(beat_t)
        
        # Right Thumb: Opposes palm and clasps around mic
        for seg, ang_y, ang_z in [(1, 0.35, -0.45), (2, 0.20, -0.55), (3, 0.15, -0.40)]:
            pb = armature.pose.bones.get(f'J_Bip_R_Thumb{seg}')
            if pb:
                pb.rotation_mode = 'QUATERNION'
                pb.rotation_quaternion = mathutils.Euler((0.0, ang_y, ang_z + grip_pulse), 'XYZ').to_quaternion()
                pb.keyframe_insert(data_path="rotation_quaternion", frame=frame)
                
        # Right Index, Middle, Ring, Little: Curled firmly around mic handle
        finger_grips = [
            ('Index',  [-0.65, -0.85, -0.75]), # Index wrapped around upper handle
            ('Middle', [-0.75, -0.90, -0.80]), # Middle wrapped firmly
            ('Ring',   [-0.80, -0.95, -0.85]), # Ring wrapped securely
            ('Little', [-0.70, -0.90, -0.80])  # Pinky curled
        ]
        for f_name, angles in finger_grips:
            for seg in range(1, 4):
                pb = armature.pose.bones.get(f'J_Bip_R_{f_name}{seg}')
                if pb:
                    pb.rotation_mode = 'QUATERNION'
                    pb.rotation_quaternion = mathutils.Euler((0.0, 0.0, angles[seg-1] + grip_pulse), 'XYZ').to_quaternion()
                    pb.keyframe_insert(data_path="rotation_quaternion", frame=frame)

        # B. LEFT HAND FINGERS: Expressive Idol Gesture (Dáng tay múa thanh thoát)
        # Smooth fan & ripple wave matching singing phrases
        gesture_flutter = math.sin(idol_cycle * 2) * 0.15
        
        # Left Thumb: Relaxed elegant pose
        for seg, ang_y, ang_z in [(1, -0.20, 0.30), (2, -0.15, 0.25), (3, -0.10, 0.20)]:
            pb = armature.pose.bones.get(f'J_Bip_L_Thumb{seg}')
            if pb:
                pb.rotation_mode = 'QUATERNION'
                pb.rotation_quaternion = mathutils.Euler((0.0, ang_y, ang_z + gesture_flutter * 0.5), 'XYZ').to_quaternion()
                pb.keyframe_insert(data_path="rotation_quaternion", frame=frame)
                
        # Left Fingers: Index & Middle extended with grace, Ring & Pinky curled gently
        left_gestures = [
            ('Index',  [-0.20 + gesture_flutter, -0.25, -0.15]), # Index pointing gracefully
            ('Middle', [-0.30 + gesture_flutter, -0.35, -0.20]), # Middle extended
            ('Ring',   [-0.55 - gesture_flutter, -0.65, -0.50]), # Ring curled
            ('Little', [-0.65 - gesture_flutter, -0.75, -0.60])  # Pinky delicately curled
        ]
        for f_name, angles in left_gestures:
            for seg in range(1, 4):
                pb = armature.pose.bones.get(f'J_Bip_L_{f_name}{seg}')
                if pb:
                    pb.rotation_mode = 'QUATERNION'
                    pb.rotation_quaternion = mathutils.Euler((0.0, 0.0, angles[seg-1]), 'XYZ').to_quaternion()
                    pb.keyframe_insert(data_path="rotation_quaternion", frame=frame)

        # 7. FLUID HAIR DYNAMICS (CHỐNG XUYÊN QUA LƯNG ÁO)
    ...
    print(">>> Model B Full Articulated Shoulders & Fingers Exported Successfully!")

if __name__ == '__main__':
    bake_shoulders_fingers_and_full_performance()
    print("=== SHOULDERS & FINGERS BAKING COMPLETE ===")
